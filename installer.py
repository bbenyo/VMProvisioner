...
def installPackage(name):
    ...
    if os.path.exists(filesSubdir):
        ...
        with tarfile.open(name+"/files.tar.gz", "w:gz") as filesTar:
            # glob iterator here just for logging the files we're adding
            fList = glob(filesSubdir+"/**", recursive=True)
            for fname in fList:
                print("      adding: "+fname)

            # Now we really add the files with a single function call
            filesTar.add(filesSubdir, arcname='', recursive=True)
        filesTar.close()
        scpFile(os.path.join(name, "files.tar.gz"), "/tmp/"+name+"-files.tar.gz")
        os.remove(os.path.join(name, "files.tar.gz"))
        oname = "/opt/"+name;
        sshCommandExec("mkdir -p "+oname+"; mv /tmp/"+name+"-files.tar.gz "+oname+"; cd "+oname+"; tar xzf "+name+"-files.tar.gz")
